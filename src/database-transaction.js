...
import {AttributeCollection, AttributeJoinedData} from './attributes';
import {tableNameForJoin} from '../models/utils';
import {replacer} from './database-object-registry';
...
export default class DatabaseTransaction {
  ...
  find(...args) { this.database.find(...args) }
  findBy(...args) { this.database.findBy(...args) }
  findAll(...args) { this.database.findAll(...args) }
  modelify(...args) { this.database.modelify(...args) }
  count(...args) { this.database.count(...args) }
  findJSONBlob(...args) { this.database.findJSONBlob(...args) }
  ...
  persistJSONBlob(id, json) {
    const JSONBlob = require('../models/json-blob');
  ...
  _writeModels(models) {
    ...
    for (const model of models) {
      ...
      ids.push(model.id);
      values.push(model.id, JSON.stringify(json, replacer));
      columnAttributes.forEach((attr) => {
    ...
    collectionAttributes.forEach((attr) => {
      ...
      if (joinedValues.length !== 0) {
        ...
        for (let slice = slicePageCount; slice >= 0; slice --) {
