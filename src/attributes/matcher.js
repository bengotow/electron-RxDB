...
class Matcher {
  ...
  evaluate(model) {
    ...
    switch (this.comparator) {
      case '=': return modelValue == matcherValue
      case '<': return modelValue < matcherValue
      case '>': return modelValue > matcherValue
      case '<=': return modelValue <= matcherValue
      case '>=': return modelValue >= matcherValue
      case 'in': return modelValue in matcherValue
      case 'contains': return modelArrayContainsValue(modelValue, matcherValue)
      case 'containsAny': return _.any matcherValue, (submatcherValue) =>
        modelArrayContainsValue(modelValue, submatcherValue)
      case 'startsWith': return modelValue.startsWith(matcherValue)
      case 'like': return modelValue.search(new RegExp(".*${matcherValue}.*", "gi")) >= 0
...
class OrCompositeMatcher extends Matcher {
  ...
  evaluate(model) {
    return this.children.some((m) => matcher.evaluate(model));
  }

  joinSQL(klass) {
    const joins = []
    for (matcher of this.children) {
  ...
  whereSQL: (klass) =>
    const wheres = this.children.map((m) => matcher.whereSQL(klass));
