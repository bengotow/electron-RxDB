...
class JSONBlobQuery extends Query {
  formatResult(objects) {
    return (objects && objects[0]) ? objects[0].json : null;
  }
}
...
class DatabaseStore extends NylasStore {
  ...
  _prettyConsoleLog(q) {
    let log = q.replace(/%/g, '%%');
    log = `color:black |||%c ${log}`;
    log = q.replace(/`(\w+)`/g, `||| color:purple |||%c$&||| color:black |||%c`);
    ...
    for (const style of Object.keys(colorRules)) {
      for (const keyword of colorRules[style]) {
        log = log.replace(new RegExp(`\\b${keyword}\\b`, 'g'), `||| ${style} |||%c${keyword}||| color:black |||%c`);
      }
    }
  ...
  _query(query, values = []) {
    return new Promise((resolve, reject) => {
      ...
      return this._db[fn](query, values, (err, results) => {
        if (err) {
  ...
  findJSONBlob(id) {
    JSONBlob = JSONBlob || require('../models/json-blob');
    return new JSONBlobQuery(JSONBlob, this).where({id}).one();
  }
